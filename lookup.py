...
def customerinfo():
    ...
    if len(customer) > 1: #if there is more than one customer
        ...
        for x in range(len(customer)): #iterate through each of the multiples of users
            print("%s - Customer ID: %s, First Name: %s, Last Name: %s, State: %s" % ((x+1), customer[x][0],customer[x][1],customer[x][2],customer[x][3]))
        
        while True: #take input and assure its an int and an acceptable id
            verifycust = input("Input desired Customer ID > ") #ask the user what number the customer is
            try:
        ...
        customer[0] = customer[order] #make the desired customer first in the list
...
# 4 - run through the functions to complete and loop the program
count = 0
while True: #loop through the whole program
    ...
    if nextcust.lower() == "n":
        print("You searched for a total of %d names.Thank you. Have a nice day." % count)
        break
